...
@dp.callback_query_handler(IsPrivate_call(), IsAdminCheck(), text="notice_all_users")
async def get_send_notice(call: types.CallbackQuery):
    ...
    await call.message.answer(f'Введите текст для отправки всем пользователям:', reply_markup=markup_4)
...
# ----------------------------КОНЕЦ БЛОКА ОТПРАВКИ КОМАНДЫ УВЕДОМЛЕНИЯ ВСЕМ ПОЛЬЗОВАТЕЛЯМ-------------------------------
