...
block_callback = CallbackData("magazin_unblocked", "user_id")

# Определяем количество пользователей на одной странице
USERS_PER_PAGE = 5


@dp.callback_query_handler(IsAdminCheck(), IsPrivate_call(), text='magazin_unblock')
...
async def show_users_page(call, users, page_number):
    ...
    await call.message.edit_text(f'Выберите <b>Магазин/Организацию</b> для <b>Разблокировки</b>:',
...
def generate_page_buttons(page_number, total_pages):
    buttons = []
    if page_number > 1:
        buttons.append(InlineKeyboardButton(text="◀️ Назад", callback_data=f"prev_unmagazin_page_{page_number}"))
    buttons.append(InlineKeyboardButton(text=f'{page_number} / {total_pages}', callback_data='total_magaz'))
    if page_number < total_pages:
        buttons.append(InlineKeyboardButton(text="▶️ Вперед", callback_data=f"next_unmagazin_page_{page_number}"))
    return buttons


# Обработка нажатия на кнопку пагинации
@dp.callback_query_handler(lambda query: query.data.startswith(('prev_unmagazin', 'next_unmagazin')))
async def process_page_button(call: types.CallbackQuery):
    page_number = int(call.data.split('_')[-1])

    if call.data.startswith('prev_unmagazin'):
        page_number -= 1
    else:
        page_number += 1
...
@dp.callback_query_handler(text_contains='magazin_unblocked')
async def unban_user(call: types.CallbackQuery):
    ...
    for user in all_shop:
        await a_commands.update_status_blocked(user_id=user.user_id, status='active', reason='no reason')
    await call.message.answer(
        f'✅ Вы успешно <b>Разблокировали</b> организацию <b>{user_magazin}</b> !')
